...
LABEL="com.qohfq.ashare-us-catalyst.public"
PLIST="$HOME/Library/LaunchAgents/$LABEL.plist"

mkdir -p "$HOME/Library/LaunchAgents" "$ROOT_DIR/logs"
chmod +x "$ROOT_DIR/scripts/run_public_site_update.sh"
...
<dict>
  ...
  <array>
    <string>$ROOT_DIR/scripts/run_public_site_update.sh</string>
  </array>
  ...
  <string>$ROOT_DIR/logs/public-launchd.out.log</string>
  <key>StandardErrorPath</key>
  <string>$ROOT_DIR/logs/public-launchd.err.log</string>
  <key>RunAtLoad</key>
  <false/>
</dict>
</plist>
...
echo "已安装：${LABEL}，每周一至周五 09:00 更新公网 Netlify 页面。"
echo "配置文件：${PLIST}"
echo "日志：${ROOT_DIR}/logs/public-launchd.out.log"
